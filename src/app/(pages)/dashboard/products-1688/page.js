...
export default function Products() {
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      const { data } = await axios.get(
        `${process.env.NEXT_PUBLIC_SERVER_URI}/api/v1/products/1688/new/products?page=${page}&perPage=${perPage}&keyword=${debouncedSearch}`
      );
  ...
  const handleAddToDb = async (product) => {
    ...
    try {
      const { data } = await axios.get(
        `${process.env.NEXT_PUBLIC_SERVER_URI}/api/v1/products/1688/product/detail/${product.id}`
      );
      if (data) {
  ...
  const ProductCard = ({ product }) => (
    <Card className="group hover:shadow-lg relative transition-all duration-300 border-red-100 hover:border-red-300">
