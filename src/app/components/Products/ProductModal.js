...
export default function ProductModal({
          ...
          setSubCategoryId({
            ...
            label: (
              <div className="flex items-center gap-1">
                <div className="w-6 h-6 rounded-full relative overflow-hidden flex items-center justify-center">
                  <Image
                    src={product.subCategoryId.image || "/placeholder.svg"}
                    layout="fill"
  ...
  const removeThumbnail = () => {
    if (typeof thumbnail === "string") {
      setDeletedImages((prevDeleted) => [...prevDeleted, thumbnail]);
    }
    setThumbnail("");
    validateStep(2);
  };
  ...
  const validateStep = (step) => {
    ...
    switch (step) {
      case 1: // Basic Info
        ...
        break;
      case 2: // Media
        isValid = !!thumbnail && variations.some((v) => v.imageURL);
        break;
      case 3: // Pricing
        isValid = !!price;
        if (sale.isActive) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (thumbnail) {
      if (thumbnail instanceof File) {
        productData.append("file", thumbnail);
      } else {
      ...
      <div className="flex flex-col gap-6 py-4">
            ...
            <div className="border-2 border-dashed border-gray-300 p-6 md:p-8 flex flex-col items-center justify-center rounded-md bg-gray-50 hover:bg-gray-100/50 transition-colors duration-200 group">
              <label className="cursor-pointer flex flex-col items-center gap-4">
                ...
                <p className="text-sm text-gray-600 text-center max-w-xs">
                  Upload the main product image. This will be the primary image
                ...
                <span className="text-white px-5 py-2.5 text-sm font-medium rounded-md bg-customRed hover:bg-red-700 transition-all duration-300 shadow-sm hover:shadow">
                  {isUpload ? "Uploading..." : "Upload Main Image"}
                </span>
                <input
                  type="file"
                  accept="image/*"
        ...
        <div className="border rounded-md p-4 bg-white shadow-sm">
          ...
          <div className="border-2 border-dashed border-gray-300 p-6 md:p-8 flex flex-col items-center justify-center rounded-md bg-gray-50 hover:bg-gray-100/50 transition-colors duration-200 group mb-4">
            <label className="cursor-pointer flex flex-col items-center gap-4">
              <div className="w-14 h-14 md:w-16 md:h-16 rounded-full bg-red-100 flex items-center justify-center text-customRed group-hover:scale-110 transition-transform duration-300">
                <Palette className="text-[28px] md:text-[32px]" />
              </div>
              <p className="text-sm text-gray-600 text-center max-w-xs">
                Upload variation images with different colors. Each image can be
                assigned a specific color.
              </p>
              <span className="text-white px-5 py-2.5 text-sm font-medium rounded-md bg-customRed hover:bg-red-700 transition-all duration-300 shadow-sm hover:shadow">
                {isUpload ? "Uploading..." : "Add Variation Image"}
              </span>
              <input
                type="file"
                accept="image/*"
                className="hidden"
                onChange={UploadVeriationImages}
                disabled={isUpload}
              />
            </label>
          </div>
          ...
          {variations && variations.some((v) => v.imageURL) ? (
            <div className="space-y-4">
              <p className="text-sm text-gray-600">
                Variation Images ({variations.filter((v) => v.imageURL).length})
              </p>
              {variations?.map((variation, index) => {
                ...
                return (
                  <div
                    key={index}
                    className="flex flex-col sm:flex-row gap-4 p-4 border border-gray-200 rounded-md bg-gray-50"
                  >
                    {/* Image */}
                    ...
                    <div className="flex-1">
                      ...
                      <Select
                        ...
                        value={colorOptions.find(
                          (c) => c.value === variation.color
                        )}
                        onChange={(selectedColor) =>
                          handleVariationColorChange(index, selectedColor)
                        }
            ...
            <div className="text-center text-gray-500 italic py-4">
              No variation images added yet. Please add at least one variation
              image.
            </div>
          )}
        </div>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-md p-4">
          <div className="flex items-start gap-2">
            ...
            <div className="text-sm text-blue-800">
              <p className="font-medium mb-1">Media Requirements:</p>
              <ul className="list-disc list-inside space-y-1 text-xs">
                <li>One main product image is required</li>
                <li>At least one variation image is required</li>
                <li>Variation images can be assigned specific colors</li>
                <li>
                  Selected variation colors will be added to available colors
                </li>
              </ul>
            </div>
          </div>
